...
This next section shows you how to take advantage of a special `fpds_csv` wrapper that acquires SBIR/STTR procurements.
...
Lets explore the data interactively again.
...
Now we can buid the trelliscope using the function we built to plot the cumulative area for each of the companies.  We also want to join in all the features we want to explore!
...
hc_items
```
#### Exploring the whole 2021 Budget

Lets see if we can explore the whole budget interactively.

To do that we will build a hierarcical sunburst visualization.
